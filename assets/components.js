...
var EventHandler = /** @class */ (function () {
    ...
    EventHandler.prototype.unregister = function (eventObject) {
        if (!eventObject)
            return null;
        var el = eventObject.el, event = eventObject.event, listener = eventObject.listener, options = eventObject.options;
        if (!el || !event || !listener)
            return null;
        var indexToRemoveFrom = this.events.findIndex(function (e) { return (e.event === event && e.listener === listener && e.options === options); });
        if (indexToRemoveFrom === -1)
            return null;
        this.events.splice(indexToRemoveFrom, 1);
...
;// ./node_modules/@pixelunion/shopify-components/components/header-password-drawer/default/header-password-drawer.js
...
;// ./node_modules/@pixelunion/shopify-components/components/add-the-look-item/default/add-the-look-item.js
/**
...
;// ./node_modules/@pixelunion/shopify-components/sections/shop-the-look/default/shop-the-look.js
/**
 * @class ShopTheLook
 * @classdesc The Shop the Look section. Displays an image
 *            with interactable points directing customers to featured products.
...
;// ./node_modules/@pixelunion/shopify-components/components/video-player/default/video-player.js
class VideoPlayer extends Core.Component {
  constructor(el, context) {
    super(el, context);

    this.section = el;
    this.videoEl = el.querySelector('[data-video]');
    this.videoType = this.data.type;
    this.videoId = this.data.id;

    if (this.videoType === 'youtube') {
      this.ytPlaceholder = this.section.querySelector('[data-youtube-placeholder]');
      this.ytAttributeString = this.ytPlaceholder.dataset.attributeString;
      this.ytTitle = this.ytPlaceholder.dataset.title;
      ...
      const observer = new IntersectionObserver((entries, observer) => {
        entries.forEach(entry => {
          if (entry.isIntersecting) {
            this._initYouTube();
            this.videoEl = el.querySelector('[data-video]');
            this._video = Core.Media.createVideo(this.videoEl, this.videoType);
            observer.disconnect();
          }
        });
      }, { threshold: 0 });
...
;// ./node_modules/@pixelunion/shopify-components/sections/testimonials/default/testimonials.js
class Testimonials extends Core.SectionComponent {
  constructor(el, context) {
    super(el, context);

    this.autoPlay = this.data.autoPlay;
    this.autoPlayDuration = this.data.autoPlayDuration;
    this.animateOutDuration = this.data.animateOutDuration;
    this.animateInDelay = this.data.animateInDelay;

    this.container = el;
    this.slider = el.querySelector('[data-testimonials-slider]');
    this.sliderSlides = [...this.slider.children];
    this.navigationCurrentSlide = el.querySelector('[data-testimonials-slider-current-slide]');
    this.navigationArrowPrevious = el.querySelectorAll('[data-testimonials-slider-previous]');
    this.navigationArrowNext = el.querySelectorAll('[data-testimonials-slider-next]');
    ...
    if (this.sliderSlides.length < 2) return;
  ...
  _registerEventListeners() {
    // controls
    this.navigationArrowPrevious.forEach(prev => {
      this.events.register(prev, 'click', e => {
        e.preventDefault();
        this.goToSlide(this.state.selectedSlideIndex - 1);
      });
    });
    this.navigationArrowNext.forEach(next => {
      this.events.register(next, 'click', e => {
        e.preventDefault();
        this.goToSlide(this.state.selectedSlideIndex + 1);
      });
    });
    ...
    // swipe gestures
    this.events.register(this.slider, 'pointerdown', e => { this._handleDragStart(e) });
    this.events.register(this.slider, 'pointermove', e => { this._handleDrag(e) });
    this.events.register(this.slider, 'pointerup', e => { this._handleDragEnd(e) });
    this.events.register(this.slider, 'pointercancel', e => { this._handleDragEnd(e) });

    // TODO: pause autoplay on hover?
    // TODO: disable text selection (firefox)?

    // theme editor events
    this.events.register(document, 'shopify:block:select', e => {
      const slideIndex = this.sliderSlides.indexOf(e.target);
      if (slideIndex > -1) {
  ...
  goToSlide(index) {
    const numSlides = this.sliderSlides.length;
    const targetIndex = (index + numSlides) % numSlides;
    const targetSlide = this.sliderSlides[targetIndex];
    const currentSlide = this.sliderSlides[this.state.selectedSlideIndex];
    const displayPosition = targetIndex + 1;
    const timeoutDuration = this.data.animateOutDuration + this.data.animateInDelay;

    if (!targetSlide || !currentSlide) return;
    this.state.selectedSlideIndex = targetIndex;
    this.sliderSlides.forEach(slide => {
      slide.classList.remove('block-quote--fade-in');
    });
    this.resetAutoPlay();

    requestAnimationFrame(() => {
      currentSlide.classList.add('block-quote--fade-out');
      setTimeout(() => {
        targetSlide.classList.add('block-quote--fade-in');
        currentSlide.classList.remove('block-quote--fade-out');
        this.slider.style.setProperty('--slide-index', targetIndex);
        this.navigationCurrentSlide.innerText = displayPosition;
      }, timeoutDuration);
    });
  }
  ...
  startAutoPlay() {
    if (this.autoPlay && this.autoPlayDuration) {
  ...
  _toggleDragIntent(addRemove) {
    if (!addRemove) return;
    if (this.state.dragDirection == 1) {
      this.navigationArrowPrevious.forEach(prev => {
        prev.classList[addRemove]('drag-intent');
      });
    } else if (this.state.dragDirection == -1) {
      this.navigationArrowNext.forEach(next => {
        next.classList[addRemove]('drag-intent');
      });
    }
  }
  ...
  _handleDrag(e) {
    if (!this.state.isDragging || typeof this.state.startX !== 'number') return;
    const isOverThreshold = (Math.abs(this.state.dragX) >= this.dragThreshold);
    this.state.dragX = e.clientX - this.state.startX;

    e.preventDefault();
    this.state.dragDirection = Math.sign(this.state.dragX);

    if (isOverThreshold) {
      this._toggleDragIntent('add');
    } else {
      this._toggleDragIntent('remove');
    }
  }

  _handleDragEnd(e) {
    if (!this.state.isDragging || !this.state.dragDirection) return;
    if (Math.abs(this.state.dragX) < this.dragThreshold) return;
    const currentIdx = this.state.selectedSlideIndex;
    const numSlides = this.sliderSlides.length;
    const isFirst = currentIdx == 0;
    const isLast = currentIdx + 1 == numSlides;

    if (isLast && this.state.dragDirection == -1) {
      this.state.selectedSlideIndex = 0;
    } else if (isFirst && this.state.dragDirection == 1) {
      this.state.selectedSlideIndex = numSlides - 1;
    } else {
      this.state.selectedSlideIndex -= this.state.dragDirection;
    }

    this.goToSlide(this.state.selectedSlideIndex);
    this._clearDrag();    
  }
  ...
  _clearDrag() {
    ...
    this.state.dragX = null;
    this.state.isDragging = false;
    this.state.dragDirection = null;
  }
}

;// ./node_modules/@pixelunion/shopify-components/blocks/block-promo-image/default/block-promo-image.js
...
;// ./node_modules/@pixelunion/shopify-components/blocks/block-promo-text/default/block-promo-text.js
...
;// ./node_modules/@pixelunion/shopify-components/blocks/block-promo-editorial/default/block-promo-editorial.js
...
;// ./node_modules/@pixelunion/shopify-components/sections/promotions-grid/default/promotions-grid.js
...
;// ./node_modules/@pixelunion/shopify-components/sections/product-recommendations/default/product-recommendations.js
/**
...
;// ./node_modules/@pixelunion/shopify-components/components/product-details/default/product-details.js
/**
...
;// ./node_modules/@pixelunion/shopify-components/sections/product-quickshop/default/product-quickshop.js
...
;// ./node_modules/@pixelunion/shopify-components/blocks/block-complementary-products/default/block-complementary-products.js
/**
...
;// ./node_modules/@pixelunion/shopify-components/components/product-gallery/default/product-gallery.js
...
class ProductGallery extends Core.Component {
  constructor(el, context) {
    ...
    this.galleryLayout = this.data.gallery_layout;
    ...
    if (this.galleryMedia.length > 1) {
      this.events.register(this.navigationThumbnails, 'click', e => {
        ...
        if (matchingMedia) {
          if (this.galleryLayout === 'thumbnails') {
            scrollIntoView(this.gallery, matchingMedia);
          } else {
            scrollIntoView(this.gallery, matchingMedia, 'vertical');
  ...
  onContextChange({ selectedVariant }) {
    ...
    if (matchingMedia) {
      if (Core.Breakpoints.min('M') && this.galleryLayout !== 'thumbnails') {
        scrollIntoView(this.gallery, matchingMedia, 'vertical');
      } else {
        scrollIntoView(this.gallery, matchingMedia);
      }
...
;// ./node_modules/@pixelunion/shopify-components/sections/product/default/product.js
...
;// ./node_modules/@pixelunion/shopify-components/sections/page-age-gate/default/page-age-gate.js
...
;// ./node_modules/@pixelunion/shopify-components/sections/main-search/default/main-search.js
...
;// ./node_modules/@pixelunion/shopify-components/components/drawer-filters/default/drawer-filters.js
...
;// ./node_modules/@pixelunion/shopify-components/components/filters-active-mobile/default/filters-active-mobile.js
/**
...
;// ./node_modules/@pixelunion/shopify-components/components/filters-buttons-mobile/default/filters-buttons-mobile.js
/**
...
;// ./node_modules/@pixelunion/shopify-components/components/faceted-filters-mobile/default/faceted-filters-mobile.js
/**
...
;// ./node_modules/@pixelunion/shopify-components/components/faceted-filters/default/faceted-filters.js
...
;// ./node_modules/@pixelunion/shopify-components/sections/main-collection-grid/default/main-collection-grid.js
/**
...
;// ./node_modules/@pixelunion/shopify-components/components/product-cross-sell-slider/default/product-cross-sell-slider.js
/**
...
;// ./node_modules/@pixelunion/shopify-components/sections/main-cart/default/main-cart.js
  ...
  _createListeners() {
    ...
    cartRows.forEach(row => {
      ...
      if (quantitySelector) {
        ...
        const quantityExceedEvent = this.events.register(
          quantitySelector.el,
          'quantity:exceed',
          e => {
            const { max } = e.detail;
            const errorEl = document.querySelector(`[data-cart-item-error="${rowKey}"]`);
            const errorText = this.data.locales.quantityError.replace('{{ quantity }}', max);
      ...
      if (buttonRemoveItem) {
        const removeClickEvent = this.events.register(buttonRemoveItem, 'click', e => {
          e.preventDefault();
          this._removeRow({ id: rowId, key: rowKey, row: row, event: 0 });
          this.events.unregister(removeClickEvent);
        });
      }
    });
...
;// ./node_modules/@pixelunion/shopify-components/sections/main-blog/default/main-blog.js
...
;// ./node_modules/@pixelunion/shopify-components/sections/image-banner/two-image/image-banner.js
/**
 * @class ImageBanner
...
class ImageBanner extends Core.SectionComponent {
  constructor(el, context) {
    ...
    this.mobileHeight = this.data.mobileHeight;
  ...
  _adjustSectionHeight() {
    ...
    const imageHeight = visibleImage.offsetHeight ?? 0;
    ...
    // Determine the desktop comparison value based on the desktop height.
    const valueToCompare = this.desktopHeight === 'auto' ? imageHeight : Number(this.desktopHeight);

    if (Core.Breakpoints.min('S') && valueToCompare < totalContainerHeight) {
      this.wrapper.style.setProperty('--image-banner-desktop-height', `${totalContainerHeight}px`);
    }

    // Determine the mobile comparison value based on the mobile height.
      const mobileValueToCompare = this.mobileHeight === 'auto' ? imageHeight : (this.mobileHeight === 'fit_screen' ? window.innerHeight : Number(this.mobileHeight));

      if (mobileValueToCompare < totalContainerHeight) {
        this.wrapper.style.setProperty('--image-banner-mobile-height', `${totalContainerHeight}px`);
      }
  }
}

;// ./node_modules/@pixelunion/shopify-components/components/navigation-drawer/default/navigation-drawer.js
/**
...
;// ./node_modules/@pixelunion/shopify-components/components/live-search/default/live-search.js
...
;// ./node_modules/@pixelunion/shopify-components/blocks/block-meganav/default/block-meganav.js
/**
 * @class BlockMeganav
...
var EventHandler = __webpack_require__(561);
...
;// ./node_modules/@pixelunion/shopify-components/components/navigation-desktop/default/scripts/navigation-desktop-parent.js
...
;// ./node_modules/@pixelunion/shopify-components/components/navigation-desktop/default/scripts/navigation-desktop-menu.js
...
;// ./node_modules/@pixelunion/shopify-components/components/navigation-desktop/default/navigation-desktop.js
...
;// ./node_modules/@pixelunion/shopify-components/blocks/block-announcement/default/block-announcement.js
/**
...
;// ./node_modules/@pixelunion/shopify-components/components/header-toolbar/default/header-toolbar.js
...
;// ./node_modules/@pixelunion/shopify-components/sections/header/default/scripts/liveSearchTakeover.js
...
;// ./node_modules/@pixelunion/shopify-components/sections/header/default/header.js
...
;// ./node_modules/@pixelunion/shopify-components/components/disclosure/default/disclosure.js
const classes = {
...
;// ./node_modules/@pixelunion/shopify-components/components/cross-border/default/cross-border.js
/**
 * @class CrossBorder
...
;// ./node_modules/@pixelunion/shopify-components/components/share/default/share.js
...
;// ./node_modules/@pixelunion/shopify-components/components/product-page/default/product-page.js
...
;// ./node_modules/@pixelunion/shopify-components/components/product-page-drawer/default/product-page-drawer.js
...
;// ./node_modules/@pixelunion/shopify-components/components/product-size-guide/default/product-size-guide.js
...
;// ./node_modules/@pixelunion/shopify-components/components/product-gallery-modal/default/product-gallery-modal.js
/**
...
;// ./node_modules/@pixelunion/shopify-components/components/media-model/default/media-model.js
/**
 * @class MediaModel
...
;// ./node_modules/@pixelunion/shopify-components/components/media-external-video/default/media-external-video.js
/**
...
;// ./node_modules/@pixelunion/shopify-components/components/media-video/default/media-video.js
/**
 * @class MediaVideo
...
;// ./node_modules/@pixelunion/shopify-components/components/featured-product-gallery/default/featured-product-gallery.js
...
;// ./node_modules/@pixelunion/shopify-components/sections/featured-product/default/featured-product.js
...
;// ./node_modules/@pixelunion/shopify-components/components/product-card/default/product-card.js
/**
 * @class ProductCard
...
;// ./node_modules/@pixelunion/shopify-components/components/product-card-slider/default/product-card-slider.js
/**
...
;// ./node_modules/@pixelunion/shopify-components/components/customer-address-form/default/customer-address-form.js
...
;// ./node_modules/@pixelunion/shopify-components/sections/cart-drawer-template/default/cart-drawer-template.js
/**
...
;// ./node_modules/@pixelunion/shopify-components/components/age-gate/default/age-gate.js
const scrollLock = __webpack_require__(130);
const isbot = __webpack_require__(287);
...
;// ./node_modules/@pixelunion/shopify-components/components/button-text/default/button-text.js
...
;// ./node_modules/@pixelunion/shopify-components/components/surface-pick-up-drawer/default/surface-pick-up-drawer.js
...
;// ./node_modules/@pixelunion/shopify-components/components/surface-pick-up/default/surface-pick-up.js
...
;// ./node_modules/@pixelunion/shopify-components/components/notification/default/notification.js
...
;// ./node_modules/@pixelunion/shopify-components/components/gift-card-recipient-form/default/gift-card-recipient-form.js
...
;// ./node_modules/@pixelunion/shopify-components/components/button-action/default/button-action.js
...
;// ./node_modules/@pixelunion/shopify-components/components/buy-buttons/default/buy-buttons.js
...
;// ./node_modules/@pixelunion/shopify-components/components/input-fieldset/default/input-fieldset.js
/**
 * @class InputFieldset
...
;// ./node_modules/@pixelunion/shopify-components/components/input-radio/default/input-radio.js
/**
 * @class InputRadio
...
;// ./node_modules/@pixelunion/shopify-components/components/input-swatch/default/input-swatch.js
/**
 * @class InputSwatch
 * @classdesc Represents an input of type radio
 *
...
;// ./node_modules/@pixelunion/shopify-components/components/option-selectable-boxes/default/option-selectable-boxes.js
/**
...
;// ./node_modules/@pixelunion/shopify-components/components/input-select/default/input-select.js
/**
 * @class InputSelect
...
;// ./node_modules/@pixelunion/shopify-components/components/option-dropdown/default/option-dropdown.js
/**
...
;// ./node_modules/@pixelunion/shopify-components/components/options-selector/default/scripts/options.js
...
;// ./node_modules/@pixelunion/shopify-components/components/options-selector/default/options-selector.js
...
;// ./node_modules/@pixelunion/shopify-components/components/variant-selector/default/variant-selector.js
...
;// ./node_modules/@pixelunion/shopify-components/components/button/default/button.js
...
;// ./node_modules/@pixelunion/shopify-components/components/product-card-swatches/default/product-card-swatches.js
/**
...
;// ./node_modules/@pixelunion/shopify-components/components/cross-sell-card/default/cross-sell-card.js
/**
 * @class CrossSellCard
...
;// ./node_modules/@pixelunion/shopify-components/components/product-cross-sell/default/product-cross-sell.js
/**
...
;// ./node_modules/@pixelunion/shopify-components/components/input-textarea/default/input-textarea.js
...
;// ./node_modules/@pixelunion/shopify-components/components/quantity-selector/default/quantity-selector.js
/**
...
class QuantitySelector extends Core.Component {
  ...
  _inputChange(e) {
    ...
    const currentTarget = e.target;
    const startValue = parseInt(this.quantityInput.value, 10);
    const min = parseInt(this.quantityInput.min, 10);
    const max = parseInt(this.quantityInput.max, 10);
    
    if (currentTarget.type === 'number') {
      if (!isNaN(max) && startValue > max) {
        const exceedEvent = new CustomEvent('quantity:exceed', { detail: { max: max } });
        this.el.dispatchEvent(exceedEvent);
        this.quantityInput.value = this.state.quantity;
        return;

      } else if (!isNaN(min) && startValue < min) {
        this.quantityInput.value = this.state.quantity;
        return;
      }

    } else if (currentTarget.name === 'minus') {
      if (startValue === min + 1) {
    ...
    } else if (currentTarget.name === 'plus') {
      if (startValue === max) {
        const exceedEvent = new CustomEvent('quantity:exceed', { detail: { max: max } });
        this.el.dispatchEvent(exceedEvent);
      } else {
    ...
    const newValue = parseInt(this.quantityInput.value, 10);
    ...
    this.inputTimer = setTimeout(() => {
      ...
      if (
        ...
        const updateEvent = new CustomEvent('quantity:update', { detail: { quantity: newValue, actionName: currentTarget.name } });
        this.el.dispatchEvent(updateEvent);
      }
    }, 500);
  }
...
;// ./node_modules/@pixelunion/shopify-components/components/lazy-img/default/scripts/parseItem.js
/**
...
;// ./node_modules/@pixelunion/shopify-components/components/lazy-img/default/scripts/roundSize.js
/**
...
;// ./node_modules/@pixelunion/shopify-components/components/lazy-img/default/scripts/getElementSize.js
/**
...
;// ./node_modules/@pixelunion/shopify-components/components/lazy-img/default/scripts/supportedDensity.js
/**
...
;// ./node_modules/@pixelunion/shopify-components/components/lazy-img/default/scripts/events.js
/**
...
;// ./node_modules/@pixelunion/shopify-components/components/lazy-img/default/scripts/loadImage.js
...
;// ./node_modules/@pixelunion/shopify-components/components/lazy-img/default/scripts/load.js
...
;// ./node_modules/@pixelunion/shopify-components/components/lazy-img/default/scripts/update.js
...
;// ./node_modules/@pixelunion/shopify-components/components/lazy-img/default/scripts/inViewport.js
/**
...
;// ./node_modules/@pixelunion/shopify-components/components/lazy-img/default/scripts/rimg.js
...
;// ./node_modules/@pixelunion/shopify-components/components/lazy-img/default/scripts/rimgShopify.js
...
;// ./node_modules/@pixelunion/shopify-components/components/lazy-img/default/lazy-img.js
...
;// ./node_modules/@pixelunion/shopify-components/components/cart-drawer/default/cart-drawer.js
...
class CartDrawer extends Core.Component {
  ...
  _createCartDrawerListeners(cartDrawerEl) {
    ...
    cartRows.forEach(row => {
        ...
        e => {
          ...
          if (quantity === 0) {
            this
              ._removeRow(row)
              .then(() => {
                this._updateLineItem(lineItemKey, 0);
                this.events.unregister(quantityChangeEvent);
              });
          } else {
      ...
      const quantityExceedEvent = this.events.register(
        ...
        e => {
          ...
          row
            ...
            .innerHTML = this.data.locales.quantityError.replace('{{ quantity }}', max);
        },
      );
      ...
      this._cartDrawerEventListeners.push(quantityChangeEvent, quantityExceedEvent, removeClickEvent);
    });
...
;// ./node_modules/@pixelunion/shopify-components/components/quickshop/default/quickshop.js
/**
 * @class Quickshop
...
;// ./node_modules/@pixelunion/shopify-components/components/flyout/default/flyout.js
/**
 * @class Flyout
...
;// ./node_modules/@pixelunion/shopify-components/components/drawer-modal/default/drawer-modal.js
/**
 * @class DrawerModal
...
;// ./node_modules/@pixelunion/shopify-components/components/site-overlay/default/site-overlay.js
/**
 * @class SiteOverlay
...
;// ./node_modules/@pixelunion/shopify-components/sections/header-password/default/header-password.js
...
;// ../../../../../tmp/paskit-KYBpQQ/scripts/components.js
...
Core.Components.register(Testimonials, 'testimonials');
...
/******/ })()
;
//# sourceMappingURL=components.js.map?1750107640816